...
function formatNutritionReply(nut, detectedLang, dbLang) {
  const labels = {
    en: { condition: "Condition", recs: "Dietary Recommendations" },
    hi: { condition: "बीमारी", recs: "आहार संबंधी सुझाव व सावधानियां" },
    bho: { condition: "बीमारी", recs: "आहार के सुझाव" },
    ta: { condition: "நிலை", recs: "உணவுப் பரிந்துரைகள்" },
    te: { condition: "పరిస్థితి", recs: "ఆహార సిఫార్సులు" },
    bn: { condition: "শারীরিক অবস্থা", recs: "আহার সংক্রান্ত পরামর্শ" }
  };
  labels.hinglish = labels.en;
  labels.tanglish = labels.en;
  labels.tenglish = labels.en;
  labels.benglish = labels.en;
  labels.pa = labels.en;
  labels.mr = labels.hi;
  labels.kn = labels.en;
  labels.ml = labels.en;
  ...
  return `🥗 **${nut.condition}**\n\n**${label.recs}:** ${nut.recommendations}`;
}
...
function handleChatRequest(message, selectedLang, conversation_id) {
    ...
    if (medData) {
      ...
      if (isMedicineTiming) {
        const timingText = {
          ...
          ta: `**${medData.name}** மருந்து எடுக்கும் அளவு மற்றும் நேரம் உங்கள் மருத்துவரின் பரிந்துரையைப் பொறுத்தது. 일반적으로 మెட்ஃபார்மின் வயிறு உபாதைகளைத் தவிர்க்க உணவுடன் எடுத்துக் கொள்ளப்படுகிறது.`,
      ...
      const labels = {
        ...
        te: { generic: "జెనరిక్ పేరు", uses: "సాధారణ ఉపయోగాలు", sideEffects: "దుష్ప్రభావాలు", precautions: "జాగ్రత్తలు", warnings: "హెచ్చరికలు" },
  ...
  const rxKeywords = ["prescription", "parcha", "rx", "image", "upload", "samjhao", "explain", "पर्चा", "விளக்கு", "விளக்கவும்", "விவరించండి", "বুঝিয়ে", "விவರಿಸಿ", "விவరించு", "రిపోర్ట్", "report", "रिपोर्ट", "cbc", "hemoglobin", "platelet"];
  let isRx = false;
  ...
  if (isRx) {
    ...
    const rxResponses = {
      ...
      ta: "தயவுசெயదు உங்கள் மருந்துச் சீட்டு அல்லது அறிக்கையின் தெளிவான படம் அல்லது PDF ஐ பதிவேற்றவும், அதை நான் ஸ்கேன் செய்து உங்களுக்கு விளக்குகிறேன்.",
  ...
  // 6. Symptom Dialog Manager (Fever, Headache, Stomach pain) - Resilient matching
  let activeSymptom = undefined;
  const feverKeywords = ["fever", "bukhar", "jor", "kachal", "jvaram", "tap", "జ్వరం", "കായച്ചൽ", "জ্বর", "காய்ச்சல்", "बुखार"];

  const hasHead = lower.includes("head") || lower.includes("sir") || lower.includes("sar") || lower.includes("matha") || lower.includes("thala") || lower.includes("tala") || lower.includes("தலை") || lower.includes("తల") || lower.includes("सिर") || lower.includes("মাথা") || lower.includes("තල");
  const hasPain = lower.includes("pain") || lower.includes("ache") || lower.includes("dard") || lower.includes("byatha") || lower.includes("vali") || lower.includes("noppi") || lower.includes("வலி") || lower.includes("నొప్పి") || lower.includes("दर्द") || lower.includes("ব্যথা") || lower.includes("ನೋವು");
  
  const hasStomach = lower.includes("stomach") || lower.includes("belly") || lower.includes("abdominal") || lower.includes("pet") || lower.includes("vayi") || lower.includes("vayitru") || lower.includes("pottikadupu") || lower.includes("வயிறு") || lower.includes("పొత్తికడుపు") || lower.includes("पेट") || lower.includes("পেট") || lower.includes("ಹೊಟ್ಟೆ");

  if (hasHead && hasPain) {
    activeSymptom = 'headache';
  } else if (hasStomach && hasPain) {
    activeSymptom = 'abdominal_pain';
  } else {
    for (const kw of feverKeywords) {
      if (lower.includes(kw)) { activeSymptom = 'fever'; break; }
    }
  }
  ...
  if (session.active_topic && (lower.includes("weakness") || lower.includes("kamjori") || lower.includes("कमजोरी") || lower.includes("नुलక") || lower.includes("சோர்வு") || lower.includes("দুর্বলতা") || isAdditionalSymptom)) {
    const activeTopicData = session.topics.find(t => t.topic === session.active_topic);
    if (activeTopicData) {
  ...
  if (lower.includes("ab kya karu") || lower.includes("ab kya kare") || lower.includes("what should i do") || lower.includes("what to do") || lower.includes("अब क्या करूं") || lower.includes("ఏం చేయాలి") || lower.includes("என்ன செய்ய வேண்டும்") || lower.includes("এখন কি করব")) {
    // Gather all symptoms across all symptom topics
    let allSymptoms = [];
    let duration = "कुछ समय";
    for (const t of session.topics) {
      if (t.topic === 'fever' || t.topic === 'headache' || t.topic === 'abdominal_pain') {
        if (t.details.symptoms) {
          allSymptoms.push(...t.details.symptoms);
        } else {
          allSymptoms.push(t.topic);
        }
        if (t.details.duration) {
    ...
    if (allSymptoms.length > 0) {
      const mappedNamesHi = allSymptoms.map(s => s === 'fever' ? 'बुखार' : s === 'abdominal_pain' ? 'पेट दर्द' : s === 'weakness' ? 'कमजोरी' : s === 'headache' ? 'सिरदर्द' : 'अन्य लक्षण');
      
      const finalReplies = {
        en: `Given your symptoms of **${allSymptoms.join(', ')}** since **${duration}**, you should rest adequately, consume light and soft foods, and stay well-hydrated. Since you are experiencing multiple symptoms, please consult a qualified physician for a diagnostic evaluation. Avoid taking pain medication on an empty stomach.`,
